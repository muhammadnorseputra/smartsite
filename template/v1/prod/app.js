...
if ($host) {
  var Uri = "".concat(window.location.origin, "/smartsite");
} else {
  var Uri = "".concat(window.location.origin);
}

var options = {
  light: "".concat(Uri, "/template/v1/prod/light.css"),
  dark: "".concat(Uri, "/template/v1/prod/dark.css"),
  startAt: "23:00",
  endAt: "06:00",
...
function changeTogglerText() {
  ...
  ModeToggler.innerHTML = currentMode === 'light' ? iconDark : iconLight;

  if (currentMode === 'light') {
    $("#navbar").addClass('border-light navbar-light bg-white').removeClass('border-dark navbar-dark bg-dark');
  } else {
    $("#navbar").addClass('border-dark navbar-dark bg-dark').removeClass('border-light navbar-light bg-white');
  }
}
// $(document).ready(function() {
