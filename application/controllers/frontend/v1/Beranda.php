...
class Beranda extends CI_Controller
{
    ...
    function template_sumber($text, $icon) {
        $html = '<div class="btn-group btn-group-sm mb-2 ml-3 ml-md-0" role="group" aria-label="button">
                    <button name="icon" type="button" class="btn btn-sm btn-light" disabled>'.$icon.'</button>
                    <button name="text-icon" type="button" class="btn btn-sm btn-light"  disabled>'.$text.'</button>
                </div>';
        return $html;
    }
            ...
            foreach ($data->result() as $row) {
                ...
                <div class="row">
                    ...
                    <a href="'.$posturl.'" class="rippler rippler-img rippler-bs-info" title="'.$row->judul.'">
                        ...
                        '.$sumber.'
                        <div class="btn-group btn-group-sm mb-2 ml-3 ml-md-0" role="group" aria-label="button">
                            <button type="button" aria-label="button" class="btn btn-sm btn-light" disabled><i class="fas fa-tag"></i></button>
                            <a href="'.$post_list_url.'" class="btn btn-sm btn-light '.$rand.'">'.$namakategori.'</a>
                        </div>
                ...
                <div>
                    ...
                    <div class="w-100">
					<button name="button-views" type="button" data-toggle="tooltip" title="Dilihat" class="btn btn-transparent border-0 rounded p-2 w-100 text-secondary"><i class="far fa-eye mr-2"></i> '.$row->views. '</button>
                    </div>
                    <div class="w-100">
					<button name="button-comments" type="button" data-toggle="tooltip" title="Komentar" class="btn btn-transparent border-0 rounded p-2 w-100 text-info"><i class="far fa-comment-alt mr-2"></i> '.$this->komentar->jml_komentarbyidberita($row->id_berita). '</button>
                    </div>
                    <div class="w-100">
                    <button name="button-shares" type="button" data-toggle="tooltip" title="Bagikan juga postingan ini" id="btn-share" data-row-id="'.$row->id_berita. '" class="btn btn-transparent  border-0 rounded p-2 w-100 text-success"><i class="fas fa-share-alt mr-2"></i> <span class="share_count">'.$row->share_count. '</span></button>
                    </div>
                    <div class="w-100">
                    <button name="button-likes" type="button" onclick="like_toggle(this)" data-toggle="tooltip" class="btn btn-transparent border-0 rounded p-2 w-100 text-danger'.$btn_like.'" title="Suka / Tidak suka" data-id-berita="' . $row->id_berita . '" data-id-user="' . $this->session->userdata('user_portal_log')['id'] . '"><i  class="'.$status_like.' fa-heart mr-2"></i> <span class="count_like">'.$row->like_count.'</span> </button>
                    </div>
					</div>
                    </div>
				</div>
