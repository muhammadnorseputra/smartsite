...
class Facebook extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('model_template_v1/M_f_facebook', 'facebook');
    }

    public function index()
    {
        $redirect_to = base_url('facebook_outh');
        $params['facebook_register_url'] = $this->facebook->create_auth_url($redirect_to);
        $params['flash'] = $this->session->flashdata('fb_flash');

        $this->load->view('Frontend/v1/pages/facebook/index', $params);
    }

    public function facebook_outh()
    {
